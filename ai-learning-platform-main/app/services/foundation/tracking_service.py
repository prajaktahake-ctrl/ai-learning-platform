...
def get_dashboard_data(db: Session, email: str):
    ...
    for path in paths:
        ...
        for topic in topics:
            ...
            # topic-level quiz (unlocked once all subtopics are completed)
            topic_quiz = db.query(Quiz).filter(
            ...
            topic_quiz_id = None
            topic_quiz_taken = False
            if topic_quiz:
                topic_quiz_id = topic_quiz.id
                topic_result = db.query(QuizResult).filter(
                    QuizResult.quiz_id == topic_quiz.id
                ).first()
                topic_quiz_taken = topic_result is not None
 
            topic_list.append({
                "topic_id": topic.id,
                "title": topic.title,
                "progress": progress,
                "quiz_id": topic_quiz_id,
                "quiz_taken": topic_quiz_taken,
